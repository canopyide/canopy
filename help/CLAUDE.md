...
If a `daintree` MCP call returns `Error [TIER_NOT_PERMITTED]: action '<id>' is not permitted for the '<tier>' tier.`, do not retry the same call. Tell the user which action was blocked and that it requires the `system` tier. To enable it, they must open Settings → Assistant → Security and turn on "Skip permission prompts", then close and reopen the help panel so the new tier is provisioned for the session.
