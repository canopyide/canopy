...
In Phase 1, Gemini help sessions are docs-only: there is no local `daintree` MCP server. Treat any guidance about inspecting live state ("How to Answer" item 2) as not applicable here — you only have `daintree-docs` and the `gh` CLI.
