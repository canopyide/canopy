...
2. **`danger` metadata must match the safeguard wired in the UI.** Action definitions and components are read together by reviewers: if `ConfirmDialog` is wired at the call site, the action definition is `danger:"confirm"`. The field has two real behavioral consumers — `ActionService.repeatLast` eligibility (`src/services/ActionService.ts:301`) and `useActionPalette` MRU exclusion (`src/hooks/useActionPalette.ts:99`) — so silent mismatches degrade both surfaces.
