...
argument-hint: "[optional filter or flags]"
---
...
- **DO NOT** wait unnecessarily — if CI is running, proceed and fix issues post-merge.
- **DO NOT** over-analyze mechanical fixes — make decisions quickly.
- **DO** fix issues yourself instead of deferring to the user.
- **DO** continue to the next PR if one fails — never stop the batch.
...
2. **Environment Check:**
   ...
   - **If dirty:** Stash changes: `git stash push -u -m "merge-prs-autostash"` and record this for restoration later.
...
- **Apply Fixes:**
  ...
  - Read files and make improvements as needed.
...
- **Handle Conflicts:**
  ...
  - **Complex:** Read conflicted files to inspect markers (`<<<<<<<`). Use your **Expert Judgment** to resolve. If you cannot resolve it safely, abort the merge (`git merge --abort`) and mark PR as FAILED.
- **Cleanup Artifacts:**
...
2. Display a single list of all processed PRs with status:
   - Format: `{status} PR #{number}: {title}`
   - MERGED = Successfully merged
   - FAILED = Failed (include brief reason in parentheses)
   - SKIPPED = Skipped (draft)

   **Do NOT use separate headers like "Merged:", "Failed:", "Skipped:".** Just show one unified list.
   Only show the status legend if there are mixed results.
