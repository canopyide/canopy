...
- **Motion Timing:** Hover and state-change transitions use 150ms `ease-out` (matching the Button base). Reserve 200ms for deliberate entry/exit animations on modals, popovers, and command palettes. Use `duration-150` (not `duration-200`) on any `transition-colors` or `transition` class that responds to hover, focus, active, or group-hover. Bare `transition-colors` (no explicit duration) inherits the app-wide default and needs no additional class.
