...
import { getMergedFlavors, getMergedFlavor, sanitizeAgentEnv } from "@/config/agents";
...
describe("Adversarial: sanitizeEnv returns {} when all entries rejected (Bug — truthy empty env)", () => {
  it("returns env=undefined (not {}) when all env values contain injection characters", () => {
    ...
    // sanitizeEnv currently returns {} instead of undefined, making flavor.env truthy
    const result = getMergedFlavors("claude", [
...
describe("Adversarial: name validation rejects apostrophes (Bug — over-aggressive XSS filter)", () => {
  it("allows flavor name with apostrophe (Don't touch)", () => {
  ...
  it('allows flavor name with double-quote ("My Config")', () => {
    const result = getMergedFlavors("claude", [{ id: "f1", name: '"My Config"' }]);
    expect(result).toHaveLength(1);
  });

  it("still blocks angle brackets (XSS-relevant)", () => {
    const result = getMergedFlavors("claude", [{ id: "f1", name: "<script>alert(1)</script>" }]);
    expect(result).toHaveLength(0);
  });
});

describe("Adversarial: getMergedFlavor with empty string flavorId", () => {
  it("returns undefined for empty-string flavorId (not the default/first flavor)", () => {
    const flavors = [{ id: "f1", name: "First Flavor" }];
    const result = getMergedFlavor("claude", "", flavors, undefined);
...
// but customFlags receives NO such check.  A CCR or custom flavor with
...
// ── adversarial: flavor.args injection (Bug — args array passes through unvalidated) ──
// validateFlavor spreads `...flavor` before overriding specific fields.
// The `args` array (used as extra CLI flags passed to the agent) is never
// checked — any string, including ones with ";" or "$(...)", survives.
// A CCR flavor or custom flavor can inject arbitrary shell commands via args.

describe("Adversarial: flavor.args injection (Bug — unsanitized args array)", () => {
...
// ── adversarial: validateFlavor whitespace-only name (Bug — check before trim) ──
// validateFlavor checks `!flavor.name` before trimming, so "   " passes the
// guard (truthy string) and then gets trimmed to "" in the return object.
// A flavor with name="" is nonsensical and should be rejected.

describe("Adversarial: validateFlavor whitespace-only name (Bug — guard before trim)", () => {
  it("rejects a flavor whose name is only whitespace", () => {
    const result = getMergedFlavors("claude", [{ id: "f1", name: "   " }]);
    expect(result).toHaveLength(0);
  });

  it("rejects a flavor whose name is a single tab character", () => {
    const result = getMergedFlavors("claude", [{ id: "f1", name: "\t" }]);
    expect(result).toHaveLength(0);
  });
