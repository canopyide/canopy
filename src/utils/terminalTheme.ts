...
export function resolveInputBarColors(theme: ITheme): InputBarColors {
  ...
  return {
    ...
    shellBg: background,
    shellBorder: `color-mix(in oklab, ${foreground} ${isDark ? "5%" : "8%"}, transparent)`,
    shellBorderHover: `color-mix(in oklab, ${foreground} ${isDark ? "8%" : "12%"}, transparent)`,
    shellBorderFocus: `color-mix(in oklab, ${accent} 20%, ${isDark ? "rgba(255,255,255,0.06)" : "rgba(0,0,0,0.08)"})`,
    shellShadow: "none",
    shellFocusRing: `color-mix(in oklab, ${accent} 10%, transparent)`,
    shellHoverBg: `color-mix(in oklab, ${background} ${isDark ? "97%" : "97%"}, ${isDark ? "white" : "black"})`,
    shellFocusBg: `color-mix(in oklab, ${background} ${isDark ? "96%" : "96%"}, ${isDark ? "white" : "black"})`,
    isDark,
  };
}
