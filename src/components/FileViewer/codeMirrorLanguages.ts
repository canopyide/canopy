// Curated registry of CodeMirror `LanguageDescription` objects for the file
// viewer. Replaces `import { languages } from "@codemirror/language-data"` so
// the per-parser chunks split out of the eager vendor-editor closure (see
// vite.config.ts vendor-editor regex). Entries without `extensions` or
// `filename` are omitted — `LanguageDescription.matchFilename` could never
// reach them, and excluding them keeps the registry tight.
...
export const CODEMIRROR_LANGUAGES: readonly LanguageDescription[] = [
  ...
  /*@__PURE__*/ LanguageDescription.of({
    ...
    extensions: ["cql"],
    load() {
      return import("@codemirror/lang-sql").then((m) => m.sql({ dialect: m.Cassandra }));
    },
  }),
  /*@__PURE__*/ LanguageDescription.of({
    name: "CSS",
  ...
  /*@__PURE__*/ LanguageDescription.of({
    ...
    extensions: ["pls"],
    load() {
      return import("@codemirror/lang-sql").then((m) => m.sql({ dialect: m.PLSQL }));
    },
  }),
  /*@__PURE__*/ LanguageDescription.of({
    name: "Python",
  ...
  /*@__PURE__*/ LanguageDescription.of({
    ...
    extensions: ["sql"],
    load() {
      return import("@codemirror/lang-sql").then((m) => m.sql({ dialect: m.StandardSQL }));
    },
  }),
  /*@__PURE__*/ LanguageDescription.of({
    name: "TSX",
  ...
  // Legacy stream parsers (each `@codemirror/legacy-modes/mode/*` is a
  // separate async chunk; order preserved so first-match semantics matches
  // upstream language-data when extensions collide, e.g. Objective-C `.m`
  // vs Octave `.m`).
  /*@__PURE__*/ LanguageDescription.of({
    name: "APL",
