...
export function FleetArmingRibbon(): ReactElement | null {
  ...
  // Commit flash: subscribe to the broadcast counter and toggle a CSS class
  // on the ribbon's root via ref. Using a ref+rAF instead of useState keeps
  // the ribbon off the React render path during a stream of keystrokes —
  // important because `noteBroadcastCommit` fires once per character.
  const broadcastSignal = useFleetArmingStore((s) => s.broadcastSignal);
  const lastObservedSignalRef = useRef(broadcastSignal);
  const commitFlashClearRef = useRef<number | null>(null);
  useEffect(() => {
    if (broadcastSignal === lastObservedSignalRef.current) return;
    lastObservedSignalRef.current = broadcastSignal;
    ...
    // Force reflow so the keyframe restarts even if the class is re-applied
    // within its own animation window. `void` reads a layout property to
    // pin the style flush.
