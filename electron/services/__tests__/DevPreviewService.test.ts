...
describe("DevPreviewService", () => {
  ...
  describe("setUrl()", () => {
    ...
    it("does nothing for non-existent session", () => {
      ...
      service.setUrl("non-existent", "http://localhost:3000");
  ...
  describe("PTY data handling", () => {
    it("extracts localhost URLs from PTY output", async () => {
      ...
      mockPtyClient.emit("data", {
        id: ptyId,
        data: "Server started at http://localhost:5173/\n",
      });
    ...
    it("transitions from installing to starting when install completes", async () => {
      vi.mocked(existsSync).mockImplementation((p: unknown) => {
        const pathStr = String(p);
        if (pathStr.includes("package.json")) return true;
        if (pathStr.includes("node_modules")) return false;
      ...
      await service.start({
        panelId: "panel-install-complete",
        cwd: "/test",
        cols: 80,
        rows: 24,
      });

      expect(service.getSession("panel-install-complete")?.status).toBe("installing");

      const session = service.getSession("panel-install-complete");
      const ptyId = session?.ptyId;

      // Simulate install completion message
      mockPtyClient.emit("data", {
        id: ptyId,
        data: "added 500 packages in 10s\n",
      });

      const startingEvent = statusEvents.find((e) => e.status === "starting");
      expect(startingEvent).toBeDefined();
    });
  });

  describe("PTY exit handling", () => {
    it("emits error status on non-zero exit code", async () => {
      vi.mocked(existsSync).mockImplementation((p: unknown) => {
        const pathStr = String(p);
        if (pathStr.includes("package.json")) return true;
        if (pathStr.includes("node_modules")) return true;
      ...
      const service = new DevPreviewServiceClass(mockPtyClient as unknown as PtyClient);
      const statusEvents: { status: DevPreviewStatus; panelId: string; message: string }[] = [];
      service.on("status", (e) => statusEvents.push(e));

      await service.start({
        panelId: "panel-exit-error",
        cwd: "/test",
        cols: 80,
        rows: 24,
      });

      const session = service.getSession("panel-exit-error");
      const ptyId = session?.ptyId;

      mockPtyClient.emit("exit", { id: ptyId, code: 1 });
      ...
      expect(service.getSession("panel-exit-error")).toBeUndefined();
    });

    it("emits stopped status on clean exit", async () => {
      vi.mocked(existsSync).mockImplementation((p: unknown) => {
        const pathStr = String(p);
        if (pathStr.includes("package.json")) return true;
        if (pathStr.includes("node_modules")) return true;
      ...
      await service.start({
        panelId: "panel-exit-clean",
        cwd: "/test",
        cols: 80,
        rows: 24,
      });

      const session = service.getSession("panel-exit-clean");
      const ptyId = session?.ptyId;

      mockPtyClient.emit("exit", { id: ptyId, code: 0 });
  ...
  describe("getSession()", () => {
    ...
    it("returns session data for existing panel", async () => {
      const service = new DevPreviewServiceClass(mockPtyClient as unknown as PtyClient);
      ...
      const session = service.getSession("panel-get");
      expect(session).toBeDefined();
      expect(session?.panelId).toBe("panel-get");
